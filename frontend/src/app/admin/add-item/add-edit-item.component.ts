...
export class AddEditItemComponent implements OnInit {
  ...
  constructor(formBuilder: FormBuilder, readonly categoriesService: CategoryService, private readonly _itemsService: ItemsService, private readonly activatedRoute: ActivatedRoute) {
  ...
  async ngOnInit() {
    ...
    const item = this._itemsService.get().pipe(map(items => items.find(i => i.id === Number(itemId))));
    if (item) {
