...
export class AuthService {
  ...
  constructor(private readonly _http: HttpClient, private _user: UserService) {
  ...
  get isAdmin() {
    return this.curentUserValue.roles.some(role => role.name.toLowerCase() === 'admin');
  }
  ...
  public async removeFav(iid: number) {
    this.curentUserValue.favouriteItems = this.curentUserValue.favouriteItems.filter(
      item => item.id !== iid,
    );
  ...
  login(username: string, password: string) {
    ...
    return this._http.post<IUser>(url, user).pipe(
      tap(saveToStorage),
      map(user => {
        this.currentUserSubject.next(user);
        console.log(user);
  ...
  register(user: IUser): Observable<IUser> {
    return this._http.post<IUser>(this._apiUrl, user).pipe(
      tap(saveToStorage),
      map(user => {
        this.currentUserSubject.next(user);
        return user;
